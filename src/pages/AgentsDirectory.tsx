...
export default function AgentsDirectory() {
  ...
  const fetchAgents = async () => {
    try {
      ...
      const response = await agentsAPI.getPublicList(params);
      if (response.success) {
      ...
      toast({
        ...
        description: err.message || "Failed to load agent directory",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900/20 flex flex-col">
      ...
      {/* Hero Header Cover */}
      <div className="py-12 md:py-20 bg-gradient-to-r from-navy via-navy-light to-secondary/80 text-white relative">
        <div className="absolute inset-0 bg-grid-pattern opacity-10" />
        <div className="container mx-auto px-4 relative z-10 text-center">
          <Link 
            to="/" 
            className="inline-flex items-center gap-1.5 text-xs text-white/80 hover:text-white mb-6 border border-white/20 hover:border-white/40 bg-white/5 py-1.5 px-3 rounded-full transition-all"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Return Home
          </Link>
          <h1 className="text-3xl md:text-5xl font-extrabold mb-4">Sourcing Agents Directory</h1>
          <p className="text-sm md:text-base text-white/80 max-w-2xl mx-auto">
            Browse, search, and connect directly with verified independent agents in Kariakoo. Pick an agent to shop products, verify cargo, and handle logistics.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12 flex-grow">
        ...
        <div className="bg-card rounded-2xl p-6 border border-border/80 shadow-md mb-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
            ...
            <div className="relative md:col-span-2">
              ...
              <Input
                type="text"
                placeholder="Search agent name, business or specialties..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="pl-10 h-11 border-border focus-visible:ring-secondary"
              />
            </div>
            ...
            <div className="relative">
              <Select value={region} onValueChange={setRegion}>
                <SelectTrigger className="h-11 border-border focus:ring-secondary">
            ...
            {/* Boutique Level Filter */}
            <div className="relative">
              <Select value={level} onValueChange={setLevel}>
                <SelectTrigger className="h-11 border-border focus:ring-secondary">
                  <SelectValue placeholder="Boutique Level" />
                </SelectTrigger>
                <SelectContent>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-card border border-border border-dashed rounded-3xl p-12 max-w-md mx-auto shadow-sm">
            <SlidersHorizontal className="w-12 h-12 text-muted-foreground mx-auto mb-4 animate-bounce-slow" />
            <h3 className="text-xl font-bold text-foreground mb-2">No Agents Found</h3>
            <p className="text-muted-foreground mb-6">
              Try adjusting your search criteria or filters to find agents matching your needs.
            </p>
            <Button onClick={() => { setSearch(""); setRegion("all"); setLevel("all"); }} variant="outline" className="border-secondary text-secondary hover:bg-secondary/10">
              Clear All Filters
            </Button>
          </div>
        ) : (
